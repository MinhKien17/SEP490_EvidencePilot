...
export default function Modal({ open, onClose, title, children, wide, closeLabel = 'Close' }) {
  const ref = useRef();
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/55 backdrop-blur-sm p-4" onClick={(e) => { if (e.target === e.currentTarget) onClose(); }}>
      <div ref={ref} role="dialog" aria-modal="true" aria-label={title} className={`bg-(--surface) text-(--text-primary) border border-(--border) rounded-2xl shadow-2xl max-h-[90vh] overflow-y-auto ${wide ? 'max-w-3xl w-full' : 'max-w-lg w-full'}`}>
