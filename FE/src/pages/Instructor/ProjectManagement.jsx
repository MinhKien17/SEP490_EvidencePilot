...
export default function ProjectManagement() {
  ...
  const handleCreate = async () => {
    ...
    setCreating(true);
    try { await api.post('/api/projects', { title: newTitle }); setShowCreate(false); setNewTitle(''); fetchProjects(); }
      ...
      {showCreate && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 backdrop-blur-sm">
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-md p-6">
            ...
            <input value={newTitle} onChange={e => setNewTitle(e.target.value)} placeholder={t.project || 'Project title...'} autoFocus className="w-full border border-slate-200 rounded-lg p-3 text-sm outline-none focus:ring-1 focus:ring-indigo-500 mb-4" />
