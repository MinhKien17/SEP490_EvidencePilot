    ...
    <header className="h-14 border-b border-(--border) bg-(--header-bg) backdrop-blur-md flex items-center px-2 sm:px-4 shrink-0 shadow-sm relative z-50">
      ...
      <div className="flex items-center gap-0.5 sm:gap-1 shrink-0">
        ...
        <div className="hidden sm:flex items-center gap-0.5 lg:gap-1">
          ...
          <button data-tour="header-ai-review" onClick={handleRunAiReview} disabled={loadingAiReview || !selectedPaper || isLocked} className="bg-(--brand) hover:bg-(--brand-hover) disabled:opacity-50 text-(--on-brand) p-2 lg:px-3 rounded-lg text-xs font-bold flex items-center gap-1.5 shadow-sm transition-colors" title={t('aiReview')}>
        ...
        <div className="relative sm:hidden">
          ...
          {showMoreMenu && (
            <div className="absolute right-0 top-full mt-2 w-[min(18rem,calc(100vw-1rem))] bg-(--surface) border border-(--border) rounded-xl shadow-xl z-[99999] overflow-hidden">
              ...
              <button onClick={() => runMobileAction(handleRunAiReview)} disabled={loadingAiReview || !selectedPaper || isLocked} className="w-full text-left px-4 py-3 text-xs font-bold text-(--brand) hover:bg-(--brand-soft) disabled:opacity-40">{loadingAiReview ? t('loading') : t('aiReview')}</button>
