...
export default function FilePanel({ isOpen, width, onResizeStart, sections, assignedSection, selectedSectionId, onSelectSection, selectedPaper, onSelectPaper, papers, onUploadPaper, sources, onUploadSource, onDeleteSource, mediaAssets, onUploadMedia, onDeleteMedia, onInsertMedia, showToast }) {
  ...
  return (
    <>
      <aside data-tour="file-panel" style={{ width }} className="bg-(--surface-secondary) border-r border-(--border) flex flex-col shrink-0 z-10 backdrop-blur-sm relative">
        <div className="px-4 py-2.5 border-b border-(--border) bg-(--surface-tertiary)/40 flex items-center justify-between gap-2">
          <select value={selectedPaper?.id || ''} onChange={(e) => { const p = papers.find(p2 => p2.id === e.target.value); if (p) onSelectPaper(p); }} className="text-xs font-bold text-(--text-primary) bg-(--surface) border border-(--border) rounded-lg px-2 py-1 max-w-[160px] truncate outline-none focus:ring-1 focus:ring-indigo-500">
            {papers.map(p => <option key={p.id} value={p.id}>{p.originalFilename}</option>)}
          </select>
          <label className="text-(--text-tertiary) hover:text-indigo-600 transition-colors cursor-pointer shrink-0" title={t('uploadNewPaper')}>
        ...
        <div className="p-2 flex-1 max-h-[45%] overflow-y-auto border-b border-(--border) custom-scrollbar">
          {sections.length === 0 ? (
          ...
          {mediaAssets.length === 0 ? (
            ...
            mediaAssets.map(m => (
              <div key={m.id} onClick={() => onInsertMedia(m.texFilename)} className="flex items-center justify-between text-xs font-medium p-2 rounded-md hover:bg-(--surface-tertiary) cursor-pointer transition-all mt-1 group text-(--text-secondary)">
                ...
                <button onClick={(e) => { e.stopPropagation(); onDeleteMedia(m.id); }} className="opacity-0 group-hover:opacity-100 hover:text-red-600 transition-all p-0.5" title={t('deleteMedia')}>
